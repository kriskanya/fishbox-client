...
(function() {
  var FISH_BOX, HTTP, Promise, a, cheerio, clear_inbox, get_body, get_content, get_email, get_fish_box, get_link, get_link_poll, gn, make_request, q, qs, request, set_fish_box, _;
  ...
  get_body = function(handle, subject, contains_text) {
    var body_text;
    body_text = q.defer();
  ...
  get_content = function(handle, subject, contains_text, attempts) {
    var link;
    if (attempts == null) {
      attempts = 0;
    }
    ...
    attempts += 1;
    if (attempts < 15) {
      get_body(handle, subject, contains_text).then(function(email) {
        return link.resolve(email);
      }, function(error) {
        if (error === 'no_matching_email' || error === '404') {
          return setTimeout(function() {
            return link.resolve(get_content(handle, subject, contains_text, attempts));
          }, 6000);
        } else {
